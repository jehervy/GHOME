...
void ActuatorsCenter::run()
{
	...
	while(true)
	{
		...
		if (iTypeMes == 2) // Correspond ˆ un ordre de pilotage
		{
			...
			if (iVirtualId != -1)
			{
				...
				cout << "Aucun capteur ne correspond ˆ ces informations :" << endl << "metric : " << iMetric << ", room : " << iRoom << "." << endl;
		}
		else
			cout << "Warning : le message reue par ActuatorCenter n'est pas un ordre de pilotage. Ce dernier sera ignorŽ." << endl;
	}
}
